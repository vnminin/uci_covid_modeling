...
old_folder_name = "2020-10-11_2020-11-15"
...
start_date <- as.Date("2020-10-17")
end_date <- as.Date("2020-11-22")
...
folder_name_irvine <-"irvine_2020-10-18_2020-11-22"
folder_name_sa <-"sa_2020-10-18_2020-11-22"
...
## Report period: `r str_c(format(model_objects_irvine$actual_first_day, "%b %d"), "-", format(model_objects_irvine$actual_last_day, "%b %d"), sep = " ")`
...
```{r everything-irvine, fig.height=10}
posterior <- oc_post_irvine %>%
  tidy_draws() %>% 
  select(-ends_with("__"), -matches("\\[\\d+\\]")) %>% 
  pivot_longer(cols = -starts_with(".")) %>% 
  select(variable = name, value) %>% 
  mutate(type = "posterior")%>%
  filter(variable!="prog2rec_rate" & variable!="prog2death_rate" &
           variable!="log_infec_dur")

#transforming R0  
posterior$value[posterior$variable=="log_R0"] <- exp(posterior$value[posterior$variable=="log_R0"])
posterior$variable[posterior$variable=="log_R0"] <- "R[0]"

#Change frac carrs to frac susceptible (don't worry about facotor level, label is correct in final graph)
posterior$value[posterior$variable=="frac_carrs"] <- 1-posterior$value[posterior$variable=="frac_carrs"]


prior <- oc_prior_irvine %>% 
  tidy_draws() %>% 
  select(-ends_with("__"), -matches("\\[\\d+\\]")) %>% 
  pivot_longer(cols = -starts_with(".")) %>% 
...
prior$value[prior$variable=="log_R0"] <- exp(prior$value[prior$variable=="log_R0"])
prior$variable[prior$variable=="log_R0"] <- "R[0]"

#Change frac carrs to frac susceptible (don't worry about facotor level, label is correct in final graph)
prior$value[prior$variable=="frac_carrs"] <- 1-prior$value[prior$variable=="frac_carrs"]

level_list <- c("frac_carrs", "frac_carrs_infec", "frac_infec_early", "R[0]", "latent_dur", "early_dur", "prog_dur",
                "IFR", "rho_death", "sqrt_phi_inv_death", "alpha_incid_0", "alpha_incid_1","sqrt_kappa_inv_incid")

label_list <- c("S[0]", "tilde(I[0])", "tilde(I[e,0])", "R[0]", "1/gamma", "1/nu[e]",  "1/nu[p]", "eta", "rho",
                "1/sqrt(phi)", "alpha[0]", "alpha[1]",   "1/sqrt(kappa)")
posterior$variable <- factor(posterior$variable, levels=level_list, labels=label_list)
prior$variable <-factor(prior$variable, levels=level_list, labels=label_list)

eplot <- bind_rows(posterior, prior) %>%
  group_by(variable, type) %>%
  median_qi() %>% 
  ggplot(aes(value, type, xmin = .lower, xmax = .upper)) +
  geom_pointinterval() +
  facet_wrap(. ~ variable, scales = "free_x", labeller = label_parsed) +
  theme(strip.text.x = element_text(size = 25)) +
  theme_bw() +
  ggtitle("Irvine Posterior Summaries: Medians & 95% Credible Intervals",
          subtitle=str_c(format(model_objects_irvine$actual_first_day, "%b %d"), "-", format(model_objects_irvine$actual_last_day, "%b %d"))) +
...
```{r everything-sa, fig.height = 10}
posterior <- oc_post_santa_ana %>%
  tidy_draws() %>% 
  select(-ends_with("__"), -matches("\\[\\d+\\]")) %>% 
  pivot_longer(cols = -starts_with(".")) %>% 
  select(variable = name, value) %>% 
...
eplot <- bind_rows(posterior, prior) %>%
  ...
  theme_bw()+
  ggtitle("Santa Ana Posterior Summaries: Medians & 95% Credible Intervals",
          subtitle=str_c(format(model_objects_irvine$actual_first_day, "%b %d"), "-", format(model_objects_irvine$actual_last_day, "%b %d")))+
  ylab("Prior/Posterior")+
  xlab("Value")
...
lumped_ochca_covid_sa %>% 
  ...
  ylab("Count") +
  scale_y_continuous(labels = comma) +
  ggtitle("Santa Ana, CA data", subtitle = str_c("Counts binned into", as.integer(lumped_ochca_covid_sa$end_date[2] - lumped_ochca_covid_sa$end_date[1]), "day periods", sep = " ")) + scale_x_date(breaks=c("10 day"), date_labels = "%b %d", )+
  theme(strip.text = element_text(size = 20))

lumped_ochca_covid_irvine %>% 
  dplyr::select(-start_date) %>% 
  pivot_longer(-end_date) %>% 
  mutate(name = str_remove(name, "new_") %>% str_to_title()) %>% 
  mutate(name = fct_relevel(name, c("Tests", "Cases", "Deaths"))) %>% 
  ...
  ylab("Count") +
  scale_y_continuous(labels = comma) +
  ggtitle("Irvine, CA data", subtitle = str_c("Counts binned into", as.integer(lumped_ochca_covid_irvine$end_date[2] - lumped_ochca_covid_irvine$end_date[1]), "day periods", sep = " ")) + scale_x_date(breaks=c("10 day"), date_labels = "%b %d", )+
  theme(strip.text = element_text(size = 20))
```
