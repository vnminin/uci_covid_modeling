...
old_folder_name = "2020-07-24_2020-08-28"
...
- Prevalence (number of infectious individuals at any time point) is declining, but is projected to stay in (`r prepped_cumulative_inc$posterior %>% filter(name == "Prevalence", t == max(t)) %>% pull(value) %>% quantile(c(0.025, 0.975)) %>% round() %>% scales::comma() %>% paste(collapse = ", ")`) on `r format(max(prepped_cumulative_inc$posterior$t), "%B %e")`.
...
More flashed out method description is in the [manuscript](https://arxiv.org/abs/2009.02654). 
