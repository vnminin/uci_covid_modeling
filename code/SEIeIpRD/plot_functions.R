...
prep_cumulative_inc <- function(model_objects, oc_post) {
  ...
  posterior <- oc_post %>%
    ...
    mutate("Cumulative Deaths" = D * model_objects$popsize) %>%
    mutate(Prevalence = round((E + IE + IP) * model_objects$popsize)) %>%
    select(.draw, t, `Cumulative Incidence`, `Cumulative Deaths`, Prevalence) %>%
  ...
  observed <- model_objects$lumped_ochca_covid_forecast %>%
    mutate("Cumulative Incidence" = cumsum(new_cases),
           "Cumulative Deaths" = cumsum(new_deaths)) %>%
    select(t = end_date, "Cumulative Incidence", "Cumulative Deaths") %>%
