...
- 
- Basic reproductive number ($R_0$), defined as the average number of secondary infections one infectious individual produces in a completely susceptible population, inside the interval (`r R0_bci[".lower"]`, `r R0_bci[".upper"]`). 
...
- The total number of infections that had occurred between `r format(prepped_cumulative_inc$posterior[[1,"t"]]-2, '%B %d, %Y')` and `r format(death_inc_summaries["Cumulative Incidence","t"], '%B %d, %Y')` falls within (`r comma(death_inc_summaries["Cumulative Incidence",".lower"])`, `r comma(death_inc_summaries["Cumulative Incidence",".upper"])`). Due to underreporting of coronavirus infections, the above range is typically larger than the number of reported cases in the same period. The number of reported  `r comma(death_inc_summaries["Cumulative Incidence","observed"])`.
...
Our approach is based on fitting a mechanistic model of SARS-CoV-2 spread to multiple sources of surveillance data. More specifically, we use daily numbers of new cases and deaths, while taking into account changes in the total number of tests reported on each day. 
